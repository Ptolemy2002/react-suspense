...
const HiddenDiv = styled.div`
    ...
    height: 0;
`;
const ShownDiv = styled.div`
    visibility: visible;
`;

export const SuspenseBoundary = partialMemo(function SuspenseBoundary({children, fallback, init}) {
    ...
    const FallbackDiv = show ? HiddenDiv : ShownDiv;
    const ChildrenDiv = show ? ShownDiv : HiddenDiv;
    ...
    return (
        ...
        <ProxyContextProvider contextClass={SuspenseContext} value={valueRef.current} proxyRef={valueRef} onChange={onChange}>
            <FallbackDiv>{fallback}</FallbackDiv>
